...
protected:
    bool beforeExeceve(ExecveParam *execveParam) override {
        char temp[PATH_MAX];
        ALOGE("beforeExeceve %s",execveParam->pathname);
...
void startIOHook(int api_level, bool needHookProperties) {
    ...
    ALOGE("startIOHook enter");
    void *handle = dlopen("libc.so", RTLD_NOW);
    if (handle) {
        ALOGE("startIOHook handle");
        ...
        ALOGE("startIOHook register_execve_handler ExecveTraceHandler");
        register_execve_handler(new ExecveTraceHandler());
#endif
        // register_execve_handler(new ExecveTraceHandler());
        ALOGE("startIOHook register_execve_handler IORelocatExecveHandler");
